...
//  Set the varaibles to the Get Parameters or if they do not exist try the session variables if those don't exist error back to QRhomework
	...
	if(isset($_GET['examactivity_id'])) {
		...
		}else if(isset($_SESSION['examactivity_id'])) {
			$examactivity_id = htmlentities($_SESSION['examactivity_id']);
		} else {
			$_SESSION['error'] = 'examactivity_id is not being read into the diplay error 34';
			header("Location: QRExam.php");
			die();
	} 
    ...
    if(isset($_GET['exam_num'])) {
		...
		}else if(isset($_SESSION['exam_num'])) {
			$exam_num = htmlentities($_SESSION['exam_num']);
		} else {
			$_SESSION['error'] = 'exam_num is not being read into the diplay error 35';
			header("Location: QRExam.php");
			die();
	} 
      if(isset($_GET['exam_code'])) {
			$exam_code = htmlentities($_GET['exam_code']);
		}else if(isset($_SESSION['exam_code'])) {
	...
	if(isset($_GET['alias_num'])) {
			...
			$_SESSION['error'] = 'alias_num is not being read into the diplay error 36';
			header("Location: QRExam.php");
			die();
	} 
	...
	if(isset($_GET['stu_name'])) {
			...
			$_SESSION['error'] = 'stu_name is not being read into the diplay error 37';
			header("Location: QRExam.php");
			die();
	} 
