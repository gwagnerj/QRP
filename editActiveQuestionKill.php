...
$score_auth = -10;  // author penalty
    ...
    if (
        isset($_POST['iid'])
        && isset($_POST['question_id'])  && isset($_POST['reject_justification'])
     ){

        // take off poionts from author and reviewers that wanted to promote  maybe no additional point
        // need to add a kill justification field to the questionactivity table
...
//     if ($id_checker[$i] != 0){
//         //? see if they rejected or approved the problem by looking in the questionactivity table
//         $sql = '
...
//? Move the question from the question tot he questiontomb table
...
// delete the entry from the question

$sql = 'DELETE FROM Question WHERE question_id = :question_id';
$stmt = $pdo->prepare($sql);
$stmt->execute(array(':question_id' => $_POST['question_id']));




//? Delete the problem from the question table
