...
// first do some error checking on the input.  If it is not OK set the session failure and send them back to QRGameIndex or getGamePblmNum.php.
   ...
   // what if they have already loged into the game once and have an entry then we should update instead of insert
        ...
        $row = $stmt -> fetch();
        if ( $row === false ) {
            $sql = "INSERT INTO Gameactivity (game_id, team_id, pin, dex, team_size, team_size_error, name, ans_b, ans_last)
				VALUES (:game_id, :team_id, :pin, :dex, :team_size, 0, :name, :ans_b, :ans_last)";
            ...
            $team_size_error = 0;
             $sql = "UPDATE `Gameactivity` 
				SET game_id = :game_id, team_id = :team_id, pin = :pin, dex = :dex, team_size = :team_size, team_size_error = :team_size_error, name = :name, ans_b = :ans_b, ans_last = :ans_last
