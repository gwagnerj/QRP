	...
	//	var	 currentclass_id = $("#current_class_dd").val();
				   ...
				   activealias = JSON.parse(activealias);
                      ...
                      for (j=0;j<5;j++){
                        ...
                        for (i=0;i<n;i++){
                          ...
                          if(j ==1){  
                               $('#alias_num_div').append('<td> this </td>') ;
                          }
                        }
                                             if(j == 0){ $('#alias_num_div').append('</tr> <tr><td> Reflect </td>') ;}
     ...
     /*               
          ...
          var assign_id = '';
				...
				}).done (function(assign_id){
                   assignidd(assign_id,report_vals);
                })
                function assignidd(assignids, callback){
					...
					var assign_ids = JSON.parse(assignids);
                     ...
                     $('#alias_num_div').append('<td>&nbsp; &nbsp; Pblm Points &nbsp;</td>') ;
						...
						for (i=0;i<n;i++){
                           assign_id = JSON.parse(assign_ids[i]);
                                 $.ajax({
                                url: 'getresults.php',
                                method: 'post',
                                data: {assign_id:assign_id,student_id:student_id}
                                ...
                                console.log('student_id  '+ student_id);
                                 ...
                                 p_num_score_net[p] = result["p_num_score_net"];
                                 p_num_score_raw[p] = result["p_num_score_raw"];
                                 survey_pts[p] = result["survey_pts"];
                                  console.log('p_num_score_net 1 '+ p_num_score_net[p]);
                      ...
                      function report_vals(n){
                          n=6;
                          for (j=0;j<n;j++){
                              console.log('p_num_score_net 3 '+ p_num_score_net[j]);

                            }
                          
                      }
                    /*   
                      $.when(assignidd).done(function(){
