...
if (isset($_POST['eactivity_id'])){
 ...
 $sql = "SELECT * FROM Eactivity
    ...
    $problem_id = $eactivity_data["problem_id"];
...
echo (' <h3>'.$student_datum["first_name"].' '.$student_datum["last_name"].' activity on problem '.$eactivity_data["alias_num"].' &nbsp; &nbsp; problem_id '.$problem_id.' dex - '.$dex.'</h3>');
//echo (' <p> eactivity_id: '.$eactivity_id.'</p>');
//echo (' <p> Problem '.$eactivity_data["alias_num"].'</p>');

//echo (' <h3> Problem_id '.$problem_id.'-'.$dex.'</h3>');

echo (' Numeric Score on Problem Before Deductions: '.$eactivity_data['p_num_score_raw']);
echo '<br>';
echo (' Late Penalty: '.$eactivity_data['late_penalty']);
echo '<br>';echo (' Survey pts: '.$eactivity_data['survey_pts']);

echo '<br>';echo (' Numeric Score Net: '.$eactivity_data['P_num_score_net']);
echo '<br>';
echo '<br>';
echo '<hr> <h2> Eactivity on Problem</h2>';
  ...
  echo ('<p>'.$student_datum["first_name"].' '.$student_datum["last_name"].'\'s Responses:</p>');          
...
foreach(range('a','j') as $v){
    ...
    if($eactivity_data[$wc_field] != null ){
       if ($eactivity_data[$wc_field]==0 && $eactivity_data[$correct_field] != 1 ){
             echo ('&nbsp;&nbsp;&nbsp;'.$v.': not attempted <br>');  
       } else {
             
             $sql = 'SELECT * FROM Eresp WHERE eactivity_id = :eactivity_id AND part_name = :part_name  ORDER by eresp_id ASC';     
              $stmt = $pdo->prepare($sql);
               $stmt -> execute(array (
               ':eactivity_id' => $eactivity_id,
               ':part_name' => $v,
               )); 
              $eresp_data = $stmt -> fetchALL();
           //  var_dump( $eresp_data);
            if ($eactivity_data[$correct_field] ==0){
                echo ('&nbsp;&nbsp;&nbsp;'.$v.': incorrect with '.$eactivity_data[$wc_field].' tries: &nbsp;');
                foreach($eresp_data as $eresp_datum){
                    echo('&nbsp;&nbsp;&nbsp;'.$eresp_datum["resp_value"].'&nbsp;at&nbsp;'.date("M j, g:i a",strtotime($eresp_datum["created_at"])));
                }
               
            } else {
               $num_tries = $eactivity_data[$wc_field] +1;
...
echo ('<hr><h2>'.$student_datum["first_name"].' '.$student_datum["last_name"].'\'s Problem Statement</h2>');
...
<head>
<meta charset="UTF-8">
<title><?php echo ($student_datum["first_name"].' '.$student_datum["last_name"].'-P'.$eactivity_data['alias_num']); ?></title>
...
echo '<h3>Eactivity on Base Case</h3>';
echo '<br>';
...
foreach(range('a','j') as $v){
    ...
    $wc_field = 'wcount_bc_'.$v;
      if($eactivity_data[$wc_field] != null ){
       if ($eactivity_data[$wc_field]==0 && $eactivity_data[$field] !=1 ){
             echo ('&nbsp;&nbsp;&nbsp;'.$v.': not attempted <br>');  
       } else {
             
             $sql = 'SELECT * FROM Bc_resp WHERE eactivity_id = :eactivity_id AND part_name = :part_name  ORDER by bc_resp_id ASC';     
              $stmt = $pdo->prepare($sql);
               $stmt -> execute(array (
               ':eactivity_id' => $eactivity_id,
               ':part_name' => $v,
               )); 
              $bc_eresp_data = $stmt -> fetchALL();
           //  var_dump( $bc_eresp_data);
            if ($eactivity_data[$field] ==0){
                echo ('&nbsp;&nbsp;&nbsp;'.$v.': incorrect with '.$eactivity_data[$wc_field].' tries: &nbsp;');
                foreach($bc_eresp_data as $bc_eresp_datum){
                    echo('&nbsp;&nbsp;&nbsp;'.$bc_eresp_datum["resp_value"].'&nbsp;at&nbsp;'.date("M j, g:i a",strtotime($bc_eresp_datum["created_at"])));
                }
               
            } else {
           echo ('&nbsp;&nbsp;&nbsp; '.$v.': correct with '.$eactivity_data[$wc_field].' tries: &nbsp;');  
            foreach($bc_eresp_data as $bc_eresp_datum){
                    echo('&nbsp;&nbsp;&nbsp;'.$bc_eresp_datum["resp_value"].'&nbsp;at&nbsp;'.date("M j, g:i a",strtotime($bc_eresp_datum["created_at"])));
                }
        }
