...
if(isset($_POST['questionwomb_id'])){
...
elseif (isset($_GET['questionwomb_id']) && $_GET['questionwomb_id'] != '0'){ 
    ...
    $_SESSION['error'] ='questionwomb_id lost in writeQuestionPromotePreview';
}
if (isset($_POST['iid'])){
    $iid = $_POST['iid'];
} 
elseif (isset($_GET['iid'])){
    ...
    $_SESSION['error'] ='iid lost in writeQuestionPromotePreview';
...
<div id = "btn_group ">
	...
	<button type="button" id = "approve_btn"  class="btn btn-success btn-lg ms-4 hide disabled ">Promote </button> 
	<button type="button" id = "approve_btn_ghost"  title = "Check all of the items below to activate approval button." class="btn btn-success btn-lg ms-4 dim ">Promote </button> 
	<button type="button" id = "reject_btn"  title = "Activate button by typing a justification for rejecting" class="btn btn-danger btn-lg ms-4 hide disabled ">Kill - Unrepairable  </button> 
	<button type="button" id = "reject_btn_ghost"  title = "Question is not worth repairing by me or someone else" class="btn btn-outline-danger btn-lg ms-4 "> <span id = "reject_text" class = "text-secondary">Kill - Unrepairable </span> </button> 
	<button type="button" id = "sendback_btn"  title = "Activate button by typing a justification for rejecting" class="btn btn-primary btn-lg ms-4 hide disabled ">Send Back  </button> 
	<button type="button" id = "sendback_btn_ghost"  title = "Question should go back to the author with a comment" class="btn btn-outline-primary btn-lg ms-4 "> <span id = "sendback_text" class = "text-secondary">Send Back </span> </button> 
</div>
<section>
...
<div id = "sendback_justification_container" class = "my-4 input-group input-group-lg hide">
	<label for = "sendback_justification"  class = " input-group input-group-lg "> Please explain to the author the issues you had with this question:</label>
	<input id = "sendback_justification" class="form-control" spellcheck ="true" name = "sendback_justification" type = "text" style = "width: 90%;" maxlength = "512" value ="" ></input>
...
<h6> &nbsp;  qw_id = <?php echo ($questionwomb_id)?></h6>
...
reject_btn.addEventListener("click",()=>{
	...
	$.ajax({
			url: 'writeQuestionKill.php',
			method: 'post',
			data: {questionwomb_id:questionwomb_id,iid:iid,reject_justification:reject_justification.value}
...
sendback_btn.addEventListener("click",()=>{
	console.log('use ajax to reject problem and add justification to data table then go to the skir')
	// console.log ('student_id',student_id,'questionwomb_id',questionwomb_id,'sendback_justification',sendback_justification.value)
	$.ajax({
			url: 'writeQuestionSendback.php',
			method: 'post',
			data: {questionwomb_id:questionwomb_id,iid:iid,reject_justification:sendback_justification.value}
...
edit_btn.addEventListener('click', () =>{
	let location = 'writeQuestionInstructor.php?iid='+iid+'&questionwomb_id='+questionwomb_id+'&check_flag=1';
	console.log ('location',location);
	 window.location.href = location;
})
check_btn.addEventListener('click', () =>{
	let location = 'writeQuestionBirth.php?iid='+iid;
	console.log ('location',location);
	 window.location.href = location;
})
approve_btn.addEventListener('click', () =>{
	...
	$.ajax({
					...
					method: 'post',
					data: {questionwomb_id:questionwomb_id,iid:iid}
